...
# =============================================================
param([switch]$Loop, [string]$AnchorDate, [switch]$Force, [switch]$Live)
# -Live: manual "refresh now" mode. Disables the Friday-freeze so Rate / FX /
# Sector / Commodity show today's LATEST values (not last Friday's snapshot).
# Used only by the dashboard's ↻ button (serve.ps1 /refresh). The 5-min auto
# timer and deploy stay non-live (Friday-frozen weekly snapshot). CDS has no
# live source, so it keeps the frozen value even in -Live. The Monday-morning
# freeze still applies (—Live does NOT bypass it; only -Force does).
...
function _FetchCommodities {
    param([bool]$Freeze = $true)
    return @(Fetch-Group $INSTRUMENTS.commodity -FreezeFriday $Freeze)
}
...
do {
    ...
    # Previous data.json — carry-forward source when a fetch fails (rate-limit).
    $prevData = $null
    if (Test-Path $DataFile) {
        try { $prevData = Get-Content $DataFile -Raw -Encoding UTF8 | ConvertFrom-Json } catch {}
    }
    $prevRate = if ($prevData -and $prevData.rate) { @($prevData.rate) } else { @() }
    ...
    $rateOrdered += @(Fetch-Group $INSTRUMENTS.rate -Previous $prevRate)
    ...
    $yahooFx = @(Fetch-Group $INSTRUMENTS.fx -FreezeFriday $false)
    $naverFxFile = Join-Path $PSScriptRoot 'fx-naver-snapshot.json'
    $naverFxMap = @{}
    # In -Live mode use Yahoo's latest FX directly; the Naver snapshot is a frozen
    # Seoul-close, so applying it would mask "today's latest".
    if ((-not $Live) -and (Test-Path $naverFxFile)) {
        try {
    ...
    # ── CDS freeze only (no live auto-fetch source — Investing returns 403) ──
    # Rate/FX/commodity/sector now update daily (like index). CDS is the only
    # indicator with no keyless API, so it keeps the weekend capture/restore.
    ...
    if ($isWeekend -and -not $Live) {
        ...
        Write-Host ("  CDS frozen ($dow capture)") -ForegroundColor Yellow
    ...
    # prevData already loaded above (rate carry-forward). Derive index/sector.
    ...
    $output = [ordered]@{
        ...
        # All Capital Market indicators update daily (each exchange's latest close).
        # CDS is the sole exception (no live API — manually updated, weekend-frozen).
        index     = @(Fetch-Group $INSTRUMENTS.index -FreezeFriday $false -Previous $prevIndex -TrimInProgress $true)
        rate      = $rateOrdered
        commodity = @(_FetchCommodities $false)
        fx        = $fxRows
        cds       = $cdsRows
        sector    = @(Fetch-Group $INSTRUMENTS.sector -FreezeFriday $false -Previous $prevSector)
    }
