...
// 주차 토글 노출 정책: 최근 DEAL_MAX_AGE_DAYS 이내 + 최대 DEAL_MAX_WEEKS 개.
const DEAL_MAX_WEEKS = 4;
const DEAL_MAX_AGE_DAYS = 31;
function pruneDealWeeks(dates) {
  const today = todayKr(); // KST YYYY-MM-DD
  const cutoffMs = Date.parse(`${today}T00:00:00Z`) - DEAL_MAX_AGE_DAYS * 86400000;
  const cutoff = new Date(cutoffMs).toISOString().slice(0, 10); // YYYY-MM-DD
  return (dates || [])
    .slice()
    .sort()
    .reverse()                  // 최신순 (YYYY-MM-DD는 사전식 정렬 = 날짜순)
    .filter(d => d >= cutoff)   // 한 달 지난 주차 폐기
    .slice(0, DEAL_MAX_WEEKS);  // 최대 4개 주차
}
...
async function loadDealsIndex() {
  try {
    ...
    // 노출 정책: 최근 ~1개월 + 최대 4개 주차만 토글로 표시.
    // 매주 새 주차가 들어오면 한 달(≈4주) 지난 가장 오래된 주차가 자동으로 사라짐.
    // (오래된 .md 파일은 디스크/배포본에 남되 화면에서만 숨김 — index.json은 손대지 않음)
    dealsState.dates = pruneDealWeeks(data.dates || []);
...
//
// DISABLED AGAIN 2026-09-20 (user request): "거래동향의 카드뉴스 섹션 삭제" — the
// 주요 거래 동향 slot re-enabled on 8/30 is off again, so no section has a
// per-article card-news slot now. 기사·밸류에이션 카드는 그대로 유지된다.
//
...
//
// `ibd: false` = shown but NOT summed into IBD. NOTE this does not mean
// "무이자부" — it means the instrument is folded into the Equity Value side of
...
function renderValuationCard(cardId, weekDate, headline, title, data) {
  ...
  const renderExtraInput = (f) => {
    ...
    // Tag text comes from the field config — never a bare "IBD 제외", which
    // would read as "무이자부" for instruments that do bear interest.
    const tag = f.tag
      ? `<span class="val-memo-tag" title="${escapeHtml(f.memo || '')}">${escapeHtml(f.tag)}</span>`
      : '';
