...
do {
    ...
    $yahooFx = @(Fetch-Group $INSTRUMENTS.fx -FreezeFriday (-not $Live))
    ...
    if ($Live) {
        # Manual live refresh: keep the freshly-fetched live Rate, do NOT capture
        # (would corrupt the Friday snapshot with an intraday value). CDS has no
        # live source, so pull the frozen CDS if a snapshot exists.
        ...
        Write-Host ("  Rate/FX/Sector/Commodity LIVE (manual refresh); CDS kept frozen") -ForegroundColor Yellow
    } elseif ($isWeekend) {
    ...
    $output = [ordered]@{
        ...
        rate      = $rateOrdered
        commodity = @(_FetchCommodities (-not $Live))
        fx        = $fxRows
        cds       = $cdsRows
        sector    = @(Fetch-Group $INSTRUMENTS.sector -FreezeFriday (-not $Live) -Previous $prevSector)
    }
