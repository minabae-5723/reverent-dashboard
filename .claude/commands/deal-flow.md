...
### 매체 다양성 (v3 강화 — 사용자 지정 2026-05-27)

🔴 **HARD RULE — 더벨 단일 의존 금지**: thebell이 단독 보도가 많은 1차 소스인 것은 맞지만 한 매체에 의존하면 사안 해석·valuation·후속 흐름이 편향됨. 다음 분포 의무:

- 같은 매체가 한 페이지(Page 5 또는 6) 안에서 **3건 이상 연속 등장 금지**
- **한 매체가 후보 10건 중 최대 4건까지** (≤40% 비중) — thebell도 예외 없음
- **최소 4개 이상 서로 다른 매체**가 후보 10건에 분포해야 함
- thebell 단독 출처 후보가 5건 이상이면 → **인베스트조선·딜사이트·한경·매경에서 추가 발굴해 교체** (페이지 4~5 확장 fetch 필수)

**의무 매체 풀** (매주 점검):
1. **thebell.co.kr** (0103) — 1순위, 단일 의존 금지
...
**자체 검증**:
- 후보 10건의 source URL 도메인을 카운트 — thebell 비중이 ≤40%인지 확인
- 인베스트조선·딜사이트·한경·매경 중 최소 3개 매체에서 후보 1건 이상 가져왔는지 확인
...
- 🔴 [ ] **매체 다양성 HARD RULE** (v3) — 후보 10건 중 한 매체 ≤4건, 최소 4개 매체 분포, thebell 단독 ≥5건이면 인베스트조선·딜사이트·한경·매경에서 추가 발굴해 교체
