---
description: 주간 Monday Brief용 Macro + 자본시장 동향 + 주요 거래 동향 큐레이션 (Macro 5~7 + 각 10개 후보 → 사용자 선택 → MD 파일 저장)
---

# Weekly Deal Flow — Monday Brief Section Curation **v4.0**

---

## ⛔⛔ v4.0 OVERRIDE — 아래 4개 룰이 이 문서의 다른 모든 서술을 이깁니다 (2026-08-22 사용자 지시)

문서 아래쪽에는 v3.2 시절 서술(LTM Q1, 전 딜 Valuation 표 의무)이 그대로 남아 있다. **충돌하면 무조건 이 블록이 우선**이다.

### 1. 섹션 구성은 종전 그대로 — 4개 섹션 유지
- 최종 MD는 **`## 1. Macro 동향` + `## 2. 자본시장 동향` + `## 3. 주요 거래 동향` + `## 4. Deal Summary`**.
- Phase 1 후보도 종전대로 **Macro 5~7건 + 자본시장 10건 + 거래동향 10건** 전부 제시한다. Page 5 카테고리·score 공식·증시 키워드 LISTUP 모두 유효.
- ⚠️ **2026-08-22 혼동 사례 기록**: 사용자의 "자본시장 카드뉴스는 아예 삭제" 지시를 섹션 삭제로 잘못 해석해 자본시장 동향 기사 4건을 지웠고, 사용자가 "자본시장동향을 아예 없애라는 게 아니라 **카드뉴스 추가란만** 없애라고. 기사는 복구"로 정정함. 이 프로젝트에서 **"카드뉴스"는 대시보드 UI 위젯 이름**(`app.js renderMacroNotesCard`)이고 기사·섹션이 아니다. 삭제된 것은 자본시장 기사별 `+ 카드뉴스` 슬롯뿐이며(`_isCapitalMarketSection` → `return false`), **MD 콘텐츠는 아무 영향 없음**.

### 2. 🔴 Valuation 기준 = LTM 2Q26 (분기·반기보고서 제출 기업)
| 대상 | 손익(P&L) | 재무상태표(BS) |
|---|---|---|
| 분기·반기보고서 제출 기업(상장사 등) | **LTM 2Q26** = FY2025 + H1'26 누적 − H1'25 누적 | **2026년 6월말 스냅샷** 그대로 |
| 비상장 외감(분기보고서 없음) | 직전 결산 **FY2025** | FY2025 기말 |

```powershell
powershell -File .\fetch-dart-valuation.ps1 -CorpCode <8자리> -CorpName <name> -LTM -Quarter 2
```
- `-Quarter` 파라미터를 신설했다(기본 2). 1=Q1(11013) / 2=반기(11012) / 3=3Q(11014).
- 🔴 **함정**: Q1(11013)은 `thstrm_amount`가 곧 누적 3개월이지만, **반기·3Q는 `thstrm_amount`가 3개월 단독 컬럼**이고 누적(6M·9M)은 **`thstrm_add_amount`**다. 잘못 쓰면 LTM 손익이 조용히 과소계상된다. 스크립트가 자동 처리하되, 수동 파싱 시 반드시 확인할 것.
- 표 헤더에 기준을 명기한다: `(corp_code XXXXXXXX, 코스닥 XXXXXX, 단위: 억원, LTM 2Q26)`
- 분기가 넘어가면 `-Quarter 3`, 사업보고서 공시 후엔 `-LTM` 없이 연간으로 전환.

### 3. 🔴 인수 대상이 외국 법인이면 Valuation 표를 만들지 않는다
- 판단 기준은 **대상(target)의 법인 소재지**. 인수자가 외국 PE여도 대상이 한국 법인이면 표를 만든다(베인캐피탈→공차코리아 = 생성, TPG→롯데렌탈 = 생성).
- 해외 법인은 DART 미등록이라 18행이 영구 공백으로 남고, 대시보드가 채울 수 없는 입력 폼만 띄운다 → 노이즈.
- 표를 뺀 대신 **가격 앵커를 본문 불릿 한 줄로** 남긴다(과거 거래가·인수가·직전 제안가·제시 기업가치·희석 지분율). 이건 "재무 원수치 나열 금지" 룰 위반이 아니다.
- **Deal Summary 표 행은 유지**한다(corp_code 칸에 `— (미국 법인)` 식 표기, 배수 `n/a`).
- Deal Summary 하단 노트에 어느 건에 표를 붙였고 어느 건을 뺐는지 한 줄로 명기.

### 4. 아래 "전 딜 Valuation 표 필수" 룰의 유효 범위
"거래동향 모든 항목에 Valuation 표" 룰은 **국내 법인 대상 딜에만** 적용된다(룰 3이 우선).

---

매주 금요일 16:00 KST 루틴. **Monday Morning Brief**의 *Macro 동향* + *자본시장 동향* + *주요 거래 동향* 섹션을 채울 기사를 큐레이션해 대시보드 `/#deals` 페이지에 주차별로 적재한다.

🔴 **Macro 동향 필수** (사용자 반복 강조 — `feedback_macro_section_in_deals.md`): Macro 동향은 절대 누락하지 말 것.
...
- 🔴 **재무상태 요약 불릿 금지 (2026-08 지시)**: 회사의 재무 원수치(매출·영업이익·EBITDA·순이익·자본총계·차입금·현금·순현금 등)를 나열/요약하는 불릿을 **넣지 않는다** — 그 정보는 `#### Valuation` 표(+배너 멀티플)로 확인 가능. 딜 인사이트(멀티플 포지셔닝·carve-out driver·PBR 프리미엄/할인 착시 등)는 유지하되 뒷받침 수치는 표에 맡기고 "상세 재무는 Valuation 표"로 갈음. (예 삭제: "FY2025 매출 1,756억·영업이익 419억·자본 1,852억·순현금 169억 → EV/Sales 5.60x·PER 38.8x·PBR 5.40x" 같은 나열 불릿.)
...
- 🔴 **리스부채 유동/비유동 분리 (2026-08 지시 / 2026-09-21 재지시)**: 재무상태표 **유동부채** 하위 리스부채는 **`유동리스부채`** 필드, **비유동부채** 하위 리스부채는 **`리스부채`** 필드에 각각 채운다(합산 금지). 둘 다 이자부부채로 Net Debt에 포함.
  - ⛔ **계정명이 아니라 BS상 위치로 판단한다.** 회사마다 표기가 제각각이다 — `리스부채`(유동/비유동 양쪽에 같은 이름) · `유동성리스부채`/`비유동성리스부채` · `기타금융부채`에 통합 등. **유동부채 블록 안에 있으면 무조건 `유동리스부채`**, 비유동부채 블록 안에 있으면 `리스부채`다.
  - ⛔ **`fetch-dart-audit.ps1`의 `current_lease`/`lease` 출력을 그대로 믿지 말 것.** 유동·비유동에 같은 `리스부채` 라벨이 쓰이면 스크립트가 **유동분을 `lease`(비유동 슬롯)에 넣고 비유동분을 통째로 누락**한다. 반드시 BS 원문의 `Ⅰ.유동부채` / `Ⅱ.비유동부채` 블록을 눈으로 확인해 두 값을 각각 집는다.
  - 예(2026-09-18 회차 실제 오류): 우아한형제들은 유동 281.7억 + 비유동 253.5억인데 `유동리스부채` 공란 · `리스부채 282`로 잘못 들어갔고 비유동 253억이 통째로 빠졌음. 굿리치도 유동 128.7억이 비유동 슬롯에 들어가고 비유동 45.8억이 누락됐음.

- 🔴 **우선주·메자닌의 이자부부채 분류 검토 의무 (2026-09-21 지시)**: 표를 채우기 전에 **부채 블록에 상환전환우선주(RCPS)·전환우선주(CPS)·상환우선주·전환사채(CB)·신주인수권부사채(BW)·파생상품부채가 있는지 반드시 스캔**한다. K-IFRS에서 상환권이 투자자에게 있는 우선주는 자본이 아니라 **금융부채**로 계상되므로, 놓치면 Net Debt·EV가 통째로 왜곡된다.
  - 스캔 키워드: `상환전환우선주` `전환우선주` `상환우선주` `우선주` `전환사채` `신주인수권부사채` `파생상품부채`. 히트가 나오면 **해당 금액이 부채 블록에 있는지 자본 블록에 있는지**를 BS에서 확인한다.
  - **부채로 계상돼 있으면 이자부부채로 보아 Net Debt에 포함**하고, 표에 `상환전환우선주` 행을 추가한다(app.js가 인식해 합산). 자본으로 계상돼 있으면 제외하되 노트에 명기한다.
  - **자본총계가 음수면 십중팔구 RCPS 부채 계상 신호**다 — 자본잠식으로 오독하지 말고 반드시 원인을 확인해 노트에 쓴다(예: 딥엑스 자본총계 −3,639억).

- 🔴 **재무상태표 단위 검증 (2026-09-21 지시)**: 감사보고서 **본문 BS·IS는 원 단위, 주석은 천원 단위**인 경우가 압도적으로 많다. 주석에서 뽑은 숫자를 본문 숫자와 나란히 쓰면 **1,000배 오류**가 난다. 같은 표 안의 현금·자산총계 같은 앵커 계정과 자릿수를 대조해 단위를 확정한 뒤 억원으로 환산한다.
  - 예(2026-09-18 회차 실제 오류): 굿리치 차입금을 주석 단위(천원)로 착각해 **452.9억으로 입력했으나 실제는 45,286,901원 = 0.45억**이었음. 같은 BS의 현금 75,733,318,536원(757억)과 자릿수를 대조했으면 즉시 걸렸을 건임.
  - **작성 전 자가검증**: 채운 값들로 `자산총계 = 부채총계 + 자본총계`가 맞아떨어지는지, Net Debt 부호가 사업 상식과 어긋나지 않는지 한 번 검산한다.
- 🔴 **감가상각비(D&A)는 주석에서 추출 (2026-08 지시)**: 손익계산서 본문엔 D&A가 없으므로 **주석의 「비용의 성격별」·「부가가치」·「감가상각비」 표**에서 감가상각비 + 무형자산상각비를 합산해 채운다. (영업손실이어도 D&A가 크면 EBITDA는 흑자일 수 있음 — 예: 해태HTB 영업 −102 + D&A 227 = EBITDA +125). **주석은 천원 단위인 경우가 많으니 본문(원 단위)과 혼동 금지.**
- 🔴 **DART 자동채우기 버튼 없음 (2026-08 지시)**: valuation 카드에서 "📥 DART 자동 채우기" 버튼은 삭제함. OpenAPI(fnlttSinglAcntAll)는 외감 비상장에 데이터가 없으므로(status 013), **감사보고서 원문을 직접 파싱하거나 수기 입력**한다. 버튼을 다시 만들지 말 것. 외감 원문: `opendart.fss.or.kr/api/document.xml?crtfc_key=KEY&rcept_no=RCP` → ZIP→XML. K-GAAP=`<TE ADELIM="2">`가 당기값(계정명이 "자 본 총 계"처럼 띄어쓰기됨), IFRS=`<TD ALIGN="RIGHT">` 첫 숫자셀이 당기값.
...
- 포함(이자부부채): 단기차입금 + 유동성장기부채(차입금·**사채 유동성대체 모두**) + 장기차입금 + **사채(비유동)** + **유동리스부채 + 리스부채(비유동) 양쪽 모두** + (해당 시) **부채로 계상된 RCPS·CPS·상환우선주** + CB/BW 부채요소. 표에 `사채`·`유동성장기부채`·`상환전환우선주` 행이 필요하면 추가(app.js가 인식해 합산).
- ⛔ **누락 3종 세트**(2026-09-18 회차에 실제로 다 틀림): ① 비유동 리스부채 ② 유동·비유동 리스부채의 슬롯 뒤바뀜 ③ 부채로 계상된 우선주. 표 채우기 전 **BS의 유동부채·비유동부채 블록을 줄 단위로 훑어** 이자부 성격 계정을 전부 열거한 뒤 시작한다.
